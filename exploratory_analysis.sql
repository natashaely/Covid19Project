...
SELECT 
  continent, MAX(CAST(Total_deaths AS INT)) AS TotalMortalities 
FROM
  PortfolioProject..CovidDeaths
WHERE 
  continent is not NULL
...
SELECT
  ...
  dea.population,
  vac.new_vaccinations, 
SUM(CONVERT(INT,vac.new_vaccinations)) OVER (PARTITION BY dea.location ORDER BY dea.location, dea.date) AS total_daily_vaccinations
FROM
  PortfolioProject..CovidDeaths dea
JOIN
  PortfolioProject..CovidVaccinations vac
...
SELECT 
  *, (total_daily_vaccinations/population)*100
FROM 
